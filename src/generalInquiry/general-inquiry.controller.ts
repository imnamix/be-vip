...
export class GeneralInquiryController {
  ...
  @Permission('General Inquiry', 'read')
  ...
  @Patch('update/:id')
  @UseGuards(AuthGuard, PermissionGuard)
  @Permission('General Inquiry', 'update')
  @ApiBody({ type: GeneralInquiryDTO })
  ...
  @Delete('delete')
  @UseGuards(AuthGuard, PermissionGuard)
  @Permission('General Inquiry', 'delete')
  @ApiBody({ type: DeleteGeneralInquiryDTO })
  async delete(@Body() body: DeleteGeneralInquiryDTO) {
    try {
      const result = await this.service.delete(body.ids) as any;
